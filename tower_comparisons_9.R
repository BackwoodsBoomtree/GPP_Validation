...
out_name <- "G:/ChloFluo/comps/tower_comparisons_9.pdf"
...
par(mfrow = c(3, 3), oma=c(1,3,2.5,0))

for (i in 1:9){
  
  op <- par(mar = c(2.5,0,0,0.5))
  ...
  if (i == 1) {
      legend("topleft", legend=c("Tower", "ChloFluo", "FluxCom", "FluxSat"),
         col=c("black", "#DC267F", "#FE6100", "#648FFF"), lty = c(1,1,1,1),
         lwd = c(2,2,2,2), ncol = 2, y.intersp = 1, cex = 0.75)
  }
  ...
  legend("topright", legend=c(cf_report, fcom_report, fsat_report),
         ...
         lwd = NA, ncol = 1, y.intersp = 1, cex = 0.75, bty = "n")

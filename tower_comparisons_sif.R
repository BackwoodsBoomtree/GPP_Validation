...
csif    <- rast(csif_file, subds = "clima_sif")
...
for (j in 1:run_n) {
  ...
  for (i in 1:9) {
    ...
    if (i_adj <= tower_df_len) {
      ...
      csif_site  <- extract_tower_var(csif, tower_loc)
      sif_site   <- extract_tower_var(sif, tower_loc)
      c3_c4_site <- extract(c3_c4, tower_loc)
      c3_c4_report <- paste0("C4: ", c3_c4_site, "%")[[2]]
      
      cf_site_month   <- to_month(cf_site, 2019)
      csif_site_month <- to_month(csif_site, 2019)
      ...
      if (all(is.na(csif_site_month)) == FALSE) {
        
        lines(x, csif_site_month, col = "#648FFF", lwd = 2)
        csif_reg    <- summary(lm(csif_site_month ~ tower_gpp_data))
        csif_r      <- round2(csif_reg$adj.r.squared, 2, FALSE)
        csif_p      <- round2(csif_reg$coefficients[2,4], 2, FALSE)
        csif_rmse   <- round2(sqrt(mean(csif_reg$residuals^2)), 2, FALSE)
        towers$csif_r[i_adj]    <- csif_r
        towers$csif_p[i_adj]    <- csif_p
        towers$csif_rmse[i_adj] <- csif_rmse
        if (csif_p < 0.05) {
          csif_report <- paste0("R2 = ", csif_r, "; RMSE = ", csif_rmse)
        } else {
          csif_report <- paste0("*R2 = ", csif_r, "; RMSE = ", csif_rmse)
        }
      } else {
        towers$csif_r[i_adj]    <- NA
        towers$csif_p[i_adj]    <- NA
        towers$csif_rmse[i_adj] <- NA
        csif_report <- NA
      }
      ...
      if (i == 1) {
        legend("topleft", legend = c("Tower", "ChloFluo", "CLIMA SIF", "SIF"),
               col=c("black", "#DC267F", "#648FFF", "gray50"), lty = c(2,1,1,2),
               lwd = 1, ncol = 2, y.intersp = 1, cex = 0.75, bty = "n", bg =)
      }
      
      legend("topright", legend = c(cf_report, csif_report, sif_report, c3_c4_report),
             text.col=c("#DC267F", "#648FFF", "gray50", "black"), lty = NA,
      ...
      k34_details           <- t(data.frame(c("BR-K34", -2.6091, -60.2093, 1999, 2006, "EBF", "Wu et al. (2015)", 
                                              "G:/SIF_comps/figs/Wu_2016/K34_GEP.csv", rep(NA, 9))))
      ...
      if (sif_p < 0.05) {
        ...
        lines(x, csif_site_month, col = "#648FFF", lwd = 2)
      csif_reg    <- summary(lm(csif_site_month ~ k34_gpp))
      csif_r      <- round2(csif_reg$adj.r.squared, 2, FALSE)
      csif_p      <- round2(csif_reg$coefficients[2,4], 2, FALSE)
      csif_rmse   <- round2(sqrt(mean(csif_reg$residuals^2)), 2, FALSE)
      towers$csif_r[i_adj]    <- csif_r
